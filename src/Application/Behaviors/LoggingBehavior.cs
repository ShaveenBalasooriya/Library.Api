...
public sealed class LoggingBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : IRequest<TResponse>
{
    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("Long-running request: {RequestName} too {ElapsedMs}ms", requestName, stopwatch.ElapsedMilliseconds);
        }

        if (response is Result { IsFailure: true } result)
        {
            _logger.LogError("Handled {RequestName} with error {ErrorCode} in {ElapsedMs}ms", requestName, result.Error.Code, stopwatch.ElapsedMilliseconds);
        }
        else
        {
